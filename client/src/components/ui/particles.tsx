...
export const Particles: React.FC<ParticlesProps> = ({
  ...
  // Read dpr inside effects so zoom changes are picked up
  ...
  // Main setup effect — only re-runs when color changes
  useEffect(() => {
    ...
    const handleResize = () => {
      // Re-init canvas on resize so zoom/window changes are handled
      if (canvasRef.current) {
        context.current = canvasRef.current.getContext("2d");
      }
      initCanvas();
  ...
  // Mouse tracking effect — updates mouse ref without touching animation
  useEffect(() => {
    if (canvasRef.current) {
