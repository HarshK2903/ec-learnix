...
export const updateProject = async (req, res) => {
    try {
        ...
        const project = await ProjectModel.findOneAndUpdate({ _id: req.params.id, userId: req.userId, isDeleted: false }, { $set: update }, { new: true });
        if (!project) {
            res.status(404).json({ message: 'Project not found' });
            return;
        }
...
export const deleteProject = async (req, res) => {
    try {
        ...
        const project = await ProjectModel.findOneAndUpdate({ _id: req.params.id, userId: req.userId, isDeleted: false }, { $set: { isDeleted: true, deletedAt: new Date() } }, { new: true });
        if (!project) {
            res.status(404).json({ message: 'Project not found' });
            return;
        }
...
export const restoreProject = async (req, res) => {
    try {
        ...
        const project = await ProjectModel.findOneAndUpdate({ _id: req.params.id, userId: req.userId, isDeleted: true }, { $set: { isDeleted: false, deletedAt: null } }, { new: true });
        if (!project) {
