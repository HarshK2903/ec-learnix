...
import { Document, Packer, Paragraph, TextRun, HeadingLevel, AlignmentType, BorderStyle, Header, Footer, PageNumber, convertInchesToTwip, } from 'docx';
import fs from 'fs';
 ...
 */
export async function buildFormattedDocx(templateType, fields, outputPath) {
...
function buildBlogPostDocx(fields) {
    ...
    // Tags
    if (tags) {
        children.push(new Paragraph({ spacing: { before: 300 } }), new Paragraph({
...
function buildJournalDocx(fields) {
    const title = getFieldValue(fields, 'title');
...
function buildBioDataDocx(fields) {
    const name = getFieldValue(fields, 'name');
    ...
    addSection('Education', education);
    if (family)
        addSection('Family Details', family);
    if (physical)
        addSection('Physical Attributes', physical);
    if (hobbies)
        addField('Hobbies & Interests', hobbies);
    if (declaration) {
        children.push(new Paragraph({ spacing: { before: 400 } }), new Paragraph({
...
function buildReportDocx(fields) {
    const title = getFieldValue(fields, 'title');
...
function buildGenericDocx(fields) {
    const children = [];
    for (const field of fields) {
        if (!field.value)
            continue;
        children.push(createHeading(field.label, HeadingLevel.HEADING_1), ...createBody(field.value));
    }
    return new Document({
        sections: [{
                properties: {
