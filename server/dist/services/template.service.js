 ...
 */
export function analyzeFields(extractedText, templateType) {
        ...
        for (const pattern of field.detectPatterns) {
            for (let i = 0; i < lines.length; i++) {
                if (pattern.test(lines[i])) {
                    ...
                    if (headerRemainder)
                        contentLines.push(headerRemainder);
                    for (let j = i + 1; j < lines.length; j++) {
                        // Stop at the next section header (a line that matches any field pattern)
                        const isNextSection = template.fields.some((f) => f.detectPatterns.some((p) => p.test(lines[j])));
                        if (isNextSection)
                            break;
                        if (lines[j].trim())
