...
export async function assertRedisReachable() {
    if (env.NODE_ENV === 'production' && !hasRedisConfig()) {
        throw new Error('Redis is not configured. Set REDIS_URL (recommended) or REDIS_HOST + REDIS_PASSWORD + REDIS_TLS=true for Upstash.');
    }
    if (env.REDIS_HOST.includes('upstash.io') && !env.REDIS_URL && !env.REDIS_PASSWORD) {
        throw new Error('Upstash requires REDIS_PASSWORD (or a full REDIS_URL). Copy the Redis URL from Upstash Console → Connect.');
    }
    ...
    client.on('error', (err) => logRedisError('startup', err));
    try {
        ...
        logRedisTarget();
        console.error('   Upstash: use the full URL from Console → Redis → Connect → Node/ioredis, e.g. rediss://default:PASSWORD@host:6379');
        throw err;
    }
    finally {
        client.disconnect();
...
export function attachBullMQErrorHandlers(emitter, label) {
    emitter.on('error', (err) => logRedisError(label, err));
}
